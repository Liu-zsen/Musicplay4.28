<template>
	<view>
		<view class="banner">
			<image class="banner-img" mode="aspectFill" :src="playlist.coverImgUrl + '?param=600y600'"></image>
			
			<view class="banner-description">{{playlist.name}}</view>
		</view>
		<view class="uni-list">
			<view @tap="getSrc(item)" class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(item,index) in playlist.tracks" :key="index">
				<view class="uni-media-list">
					<image lazy-load="true" class="uni-media-list-logo" :src="item.al.picUrl+'?param=100y100'"></image>
